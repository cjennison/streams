...
function userBasinList(user,cb) {
  ...
  fs.readFileSync(user.dir+'/basin/aliaslist.json',function(err,data){
    if(err) {
      mkdirp(user.dir+'/basin');
      data = '{"list":[]}';
      fs.writeFileSync(user.dir+'/basin/aliaslist.json', data, function(err){
        if(err) {
          console.log("alisa not exists. cannot create list");
          cb({msg:"alisa not exists. cannot create list"});
        }
      });
    }else{
...
function addBasin_IDAlias(user,pair,cb){
  ...
  fs.readFileSync(user.dir+'/basin/aliaslist.json',function(err,data){
    ...
    fs.writeFileSync(user.dir+'/basin/aliaslist.json', JSON.stringify(ls), function(err){
      if(err){
        if(cb){
