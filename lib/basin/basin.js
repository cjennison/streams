...
var delineateBasin = function (lat, lng, cb) {
  ...
  var RArg = ['delineate_basin.R ', '{ lat :'+ lat+', long:'+ lng+', basindir:'+BASIN_DIR+', datadir:'+DATA_DIR+'}'];
  ...
  rs.stdout.on('data',function(data){
	...
	if(str[0] === 'basin') {
		...
		cb(undefined, str[1]);	// use the call back function to return the response.
		rs["basinID"] = str[1];
    console.log("lib/basin/basin.js: basinID = " + rs["basinID"]);
	}else{
    cb({msg: "cannot get basin ID"})
  }
  });
  ...
  rs.stdout.on('end',function(data){
    ...
    if(rs["basinID"]){
      ...
      cb("no input from delineate_basin.R");
    }
  });
};
