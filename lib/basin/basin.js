...
<<<<<<< HEAD
	if(!pair||!pair.basin_id||!pair.basin_alias||!user){
		console.log("the ID-alias was not passed into the function!!!!-please check the id-alias object is valid \n !pair||!pair.basin_id||!pair.basin_alias||!user");
		cb("the ID-alias does not exists!!!!");
		return ;
	}
...
=======
  if(!pair||!pair.basin_id||!pair.basin_alias||!user){
    console.log("the ID-alias does not exists!!!! \n !pair||!pair.basin_id||!pair.basin_alias||!user");
    cb("the ID-alias does not exists!!!!");
    return ;
  }
  
  console.log(JSON.stringify(pair));
>>>>>>> 0077992580efad0e425349f17ca67abb6347b07c
  fs.readFile(user.dir+'/basin/aliaslist.json',function(err,data){
    var ls = {list:[]};
    if(!err){
      ls = JSON.parse(data);
