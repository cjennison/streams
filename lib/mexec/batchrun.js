...
var saveModelTree = function(runs,user){
	...
	if(!fs.existsSync(path)){
		mkdirp(user.dir+'/tree',function(err){
			if(!err){ //new file
				...
				for(var i=0;i<runs.length;i++){
					var Arun =runs[i];
					var id = runs["basinid"];
					for(var pre in runs["preceding"]){
						id = runs["preceding"][pre+"_runid"];
						break;
					}
					var node ={
		...
		mt = fs.readFileSync(user.dir+'/tree/modelTrees.json');
		mt = JSON.parse(mt);
		if(!mt[runs[0].basinid]){
			mt[basinid] = {
...
function searchMTNode(tree,lev,id){
	...
	if(lev ===1){
		...
		for(var i=0;i<tree.children.length;i++){
			var tree = searchMTNode(tree.children[i],lev-1,id);
			if(tree) return tree;
		}
		return false;
	}
