...
Runs.prototype.getRuns = function (cb) {
  ...
  fs.readdir(this.user.dir, function (err, script_list) {
    if (err) {
      cb(err);
    }
    else {
      for (var s = 0; s < script_list.length; s++) {
        ...
        if (script !== 'config') {
          ...
          fs.readdir(this.user.dir + '/' + script, function (err, run_list) {
            if (err) {
              cb(err);
            }
            else {
              for (var r = 0; r < run_list.length; r++) {
                var run = new Run(this.user, script, run_list[r]);
                this.runs.push(run);
              }
              cb(undefined, this.runs);
            }
          });
        }
      }
